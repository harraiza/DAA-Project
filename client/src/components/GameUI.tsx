  ...
  return (
    <>
      ...
      <div className="fixed top-32 right-8 z-20 flex flex-col items-center">
        ...
        {/* Persistent Next Level Button */}
        <button
          className={`mt-6 px-4 py-2 rounded-lg font-semibold shadow transition-colors text-white ${
            state.levels.find(l => l.id === level.id + 1 && l.isUnlocked)
          ...
          disabled={!state.levels.find(l => l.id === level.id + 1 && l.isUnlocked)}
        >
          Next Level
        </button>
      </div>
